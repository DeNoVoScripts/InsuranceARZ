...
--os.remove(dirml.."/InsuranceARZ/main/update.txt")
--os.remove(dirml.."/InsuranceARZ/main/update.med")
		...
		imgui.Begin(fa.ICON_HEARTBEAT .. " MedicalHelper "..scr.version, mainWin, imgui.WindowFlags.NoResize);
			...
			if select_menu[7] then
				...
				imgui.BeginChild("about", imgui.ImVec2(0, 0), true)
					...
					imgui.Bullet()
					imgui.TextColoredRGB("Разработчик - {FFB700}Christian Costa")
					imgui.Bullet()
...
function funCMD.update()
	...
	local url = "https://raw.githubusercontent.com/DeNoVoScripts/InsuranceARZ/main/MedicalHelper.lua?raw=true"
	...
	downloadUrlToFile(url, dir, function(id, status, p1, p2)
		if status == dlstatus.STATUSEX_ENDDOWNLOAD then
			if updates == nil then 
				...
				setClipboardText("https://raw.githubusercontent.com/DeNoVoScripts/InsuranceARZ/main/MedicalHelper.lua?raw=true")
				updWin.v = false
			end
		end
		if status == dlstatus.STATUS_ENDDOWNLOADDATA then
...
function funCMD.updateCheck()
	sampAddChatMessage("{FFFFFF}[{EE4848}MedicalHelper{FFFFFF}]: Проверяем наличие обновлений...", 0xEE4848)
		local dir = dirml.."/InsuranceARZ/main/update.med"
		local url = "https://raw.githubusercontent.com/DeNoVoScripts/InsuranceARZ/main/update.med"
		downloadUrlToFile(url, dir, function(id, status, p1, p2)
			if status == dlstatus.STATUS_ENDDOWNLOADDATA then
				lua_thread.create(function()
				wait(1000)
				if doesFileExist(dirml.."/InsuranceARZ/main/update.med") then
					local f = io.open(dirml.."/InsuranceARZ/main/update.med", "r")
		...
		local dir = dirml.."/InsuranceARZ/main/update.txt"
		local url = "https://raw.githubusercontent.com/DeNoVoScripts/InsuranceARZ/main/update.txt"
		downloadUrlToFile(url, dir, function(id, status, p1, p2)
			if status == dlstatus.STATUS_ENDDOWNLOADDATA then
				lua_thread.create(function()
					wait(1000)
					if doesFileExist(dirml.."/InsuranceARZ/main/update.txt") then
					local f = io.open(dirml.."/InsuranceARZ/main/update.txt", "r")
					updinfo = f:read("*a")
					f:close()
					end
				end)
